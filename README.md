...
| Algorithms | Big-O, stack, queue, dictionary, sorting, binary search, graph BFS | Choose data structures by access pattern / 根据访问模式选择数据结构 | `examples/csharp/02-data-structures/` |
...
| Distributed systems | Idempotency, retry, timeout, circuit breaker, cache-aside | Control duplicate work and partial failure / 控制重复请求和局部故障 | `examples/csharp/07-distributed-patterns/` |
...
| Browser | Semantic HTML, CSS layout, DOM events, fetch | Learn the platform before relying on React / 先理解浏览器平台再使用 React | `examples/browser/` |
